...
def on_connected(client: NewClient, _: ConnectedEv):
    ...
    for group in groups:
            ...
            # write output of group variable to a file named groups.txt
...
async def send_message(request: SendMessageRequest):
    ...
    try:
        ...
        if client.is_on_whatsapp(request.phone_number):
            ...
            print(f"⚠️  {request.phone_number} is NOT on WhatsApp. Message will fail to send.")
...
async def send_group_message(request: SendMessageRequest):
    ...
    - **phone_number**: Group ID with country code (no + or spaces)
    - **message**: The message text to send
    """
    if not connection_status["connected"]:
        raise HTTPException(
    ...
    try:
        # Build JID and send message
        # jid = build_jid(request.phone_number, is_group=True)
        phone_number=request.phone_number

        asb= "120363408920420601"
        server = "g.us"
        jid = build_jid(asb,server=server)
        client.send_message(jid, request.message)
        return SendMessageResponse(
            status="success",
